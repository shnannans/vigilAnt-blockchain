...
contract VigilAnt is ChainlinkClient, ConfirmedOwner {
    ...
    uint256 public constant MIN_POOL_BALANCE = 5 * 1e6;         // 5 USDC — minimum pool balance for admin top-up
    ...
    /**
     * @notice  Top up a country pool from the admin wallet to meet MIN_POOL_BALANCE.
     *
     * @dev     Calculates the exact deficit: MIN_POOL_BALANCE - poolBalances[countryCode].
     *          Transfers only that amount — no excess, no fee.
     *          Requires prior USDC.approve(contractAddress, deficit) from the admin wallet.
     *          Reverts if the pool already meets or exceeds MIN_POOL_BALANCE.
     *
     * @param   countryCode  1=JPN 2=THA 3=PHL 4=IDN 5=VNM
     */
    function adminTopUp(uint8 countryCode)
        external
        onlyOwner
        validCountry(countryCode)
    {
        uint256 current = poolBalances[countryCode];
        require(current < MIN_POOL_BALANCE, "VigilAnt: pool already meets minimum balance");

        uint256 deficit = MIN_POOL_BALANCE - current;

        require(
            usdc.transferFrom(msg.sender, address(this), deficit),
            "VigilAnt: USDC transfer failed"
        );
        poolBalances[countryCode] += deficit;

        emit Deposited(msg.sender, countryCode, deficit, 0);
    }
